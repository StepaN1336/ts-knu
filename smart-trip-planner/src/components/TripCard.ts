import type { Trip } from "../models/types.js";

export function renderTripCard(
  trip: Trip,
  durationDays: number,
  ...
  card.querySelector(".js-edit-trip")?.addEventListener("click", (e: Event) => {
    e.preventDefault();
    onEdit(trip.id);
  });

  card.querySelector(".js-delete-trip")?.addEventListener("click", (e: Event) => {
    e.preventDefault();
    onDelete(trip.id);
  });
