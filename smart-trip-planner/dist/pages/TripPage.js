...
export class TripPage {
    ...
    renderDayPlan(dp, tripId) {
        const dateLabel = new Date(dp.date).toLocaleDateString("uk-UA", {
            weekday: "long",
            day: "numeric",
            month: "long",
        });
        const activitiesHtml = dp.activities.length === 0
    ...
    bindDayEvents(tripId) {
        this.container.querySelectorAll(".js-add-activity").forEach((btn) => {
            btn.addEventListener("click", () => {
                const dayId = btn.dataset["dayId"];
                this.openActivityModal(tripId, dayId);
            });
        });
        this.container.querySelectorAll(".js-delete-activity").forEach((btn) => {
            btn.addEventListener("click", () => {
                const { tripId: tId, dayId, activityId } = btn.dataset;
                this.tripService.deleteActivity(tId, dayId, activityId);
                this.render(tripId);
            });
        });
    }
    openActivityModal(tripId, dayPlanId) {
        const overlay = document.querySelector("#modal-activity") ??
    ...
    `;
        renderActivityForm(overlay.querySelector("#act-form-body"), (data) => {
...
//# sourceMappingURL=TripPage.js.map