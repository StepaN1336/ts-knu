...
async function loadCategory(shortname, _displayName) {
    const contentEl = document.getElementById("content");
    contentEl.innerHTML = `<div class="loader"><span class="loader-dot"></span><span class="loader-dot"></span><span class="loader-dot"></span></div>`;
    try {
      ...
      <section class="products-section">
        ...
        for (const product of data.items) {
            const imgUrl = getPlaceholderImage(product.shortname);
            html += `
...
// Допоміжні функції
function getPlaceholderImage(shortname) {
    const colors = {
    ...
    const colorPair = colors[shortname] ?? "2d3436/dfe6e9";
    return `https://placehold.co/180x180/${colorPair}?text=${encodeURIComponent(shortname)}`;
}
function getCategoryIcon(shortname) {
    const icons = {
        laptops: "💻",
        phones: "📱",
        headphones: "🎧",
